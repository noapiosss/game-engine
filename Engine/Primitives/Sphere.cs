...
namespace Rendering.Engine.Primitives
{
    public class Spere : Primitive
    {
        ...
        public Spere(Vector3 center, float radius, int stacks, int slices, Color4 color, Vector3 velocity, float restitution, float density, bool isStatic)
            : base(new(center, new()), color, velocity, density, restitution, isStatic)
        {
            Pivot = new(center, new());
            Radius = radius;
            Volume = 4 * MathF.PI * radius * radius * radius / 3;
            Mass = density * Volume;
            ...
            {
                float theta = i * MathF.PI / stacks;
                float sinTheta = MathF.Sin(theta);
                float cosTheta = MathF.Cos(theta);

                for (int j = 0; j <= slices; j++)
                {
                    float phi = j * 2.0f * MathF.PI / slices;
